package com.practice.leetcode.bitwise;

import org.junit.jupiter.api.Test;
import static org.junit.jupiter.api.Assertions.*;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;

/**
 * ╔═══════════════════════════════════════════════════════════════════════════╗
 * ║          BIT MANIPULATION PRACTICE - EASY TO MEDIUM                       ║
 * ╠═══════════════════════════════════════════════════════════════════════════╣
 * ║ Từ dễ đến khó, giải thích chi tiết cho người mới                          ║
 * ╚═══════════════════════════════════════════════════════════════════════════╝
 *
 * ROADMAP (theo thứ tự nên làm):
 * ───────────────────────────────
 * 1. LC 136 - Single Number (Easy)           - XOR cơ bản
 * 2. LC 191 - Number of 1 Bits (Easy)        - n & (n-1) pattern
 * 3. LC 231 - Power of Two (Easy)            - n & (n-1) pattern
 * 4. LC 268 - Missing Number (Easy)          - XOR với index
 * 5. LC 190 - Reverse Bits (Easy)            - Get/Set bit
 * 6. LC 338 - Counting Bits (Easy)           - DP + bit
 * 7. LC 461 - Hamming Distance (Easy)        - XOR + count
 * 8. LC 78  - Subsets (Medium)               - Bitmask enumeration
 * 9. LC 260 - Single Number III (Medium)     - Advanced XOR
 */
public class BitManipulationPractice {

  // ═══════════════════════════════════════════════════════════════════════════
  // #1. LC 136 - SINGLE NUMBER (Easy) ⭐ BẮT ĐẦU TỪ ĐÂY
  // ═══════════════════════════════════════════════════════════════════════════
  /**
   * ┌─────────────────────────────────────────────────────────────────────────────┐
   * │ ĐỀ BÀI:                                                                     │
   * │ Mảng có các số xuất hiện 2 lần, CHỈ 1 số xuất hiện 1 lần.                   │
   * │ Tìm số đó. Yêu cầu: O(n) time, O(1) space.                                  │
   * ├─────────────────────────────────────────────────────────────────────────────┤
   * │                                                                             │
   * │ VÍ DỤ: [4, 1, 2, 1, 2] → Output: 4                                          │
   * │                                                                             │
   * │ KEY INSIGHT:                                                                │
   * │ • XOR có tính chất: a ^ a = 0 (số giống nhau XOR = 0)                       │
   * │ • XOR có tính chất: a ^ 0 = a (XOR với 0 giữ nguyên)                        │
   * │ • XOR có tính giao hoán: a ^ b ^ a = a ^ a ^ b = 0 ^ b = b                  │
   * │                                                                             │
   * │ → XOR tất cả các số: các cặp triệt tiêu, còn lại số duy nhất!               │
   * │                                                                             │
   * │ MINH HỌA:                                                                   │
   * │ [4, 1, 2, 1, 2]                                                             │
   * │   4 ^ 1 ^ 2 ^ 1 ^ 2                                                         │
   * │ = 4 ^ (1 ^ 1) ^ (2 ^ 2)   (nhóm lại)                                        │
   * │ = 4 ^ 0 ^ 0                                                                 │
   * │ = 4                                                                         │
   * └─────────────────────────────────────────────────────────────────────────────┘
   */
  public int singleNumber(int[] nums) {
    int result = 0;
    for (int num : nums) {
      result ^= num;  // XOR tất cả
    }
    return result;
  }

  @Test
  void testSingleNumber() {
    assertEquals(1, singleNumber(new int[]{2, 2, 1}));
    assertEquals(4, singleNumber(new int[]{4, 1, 2, 1, 2}));
    assertEquals(1, singleNumber(new int[]{1}));
  }

  // ═══════════════════════════════════════════════════════════════════════════
  // #2. LC 191 - NUMBER OF 1 BITS (Easy)
  // ═══════════════════════════════════════════════════════════════════════════
  /**
   * ┌─────────────────────────────────────────────────────────────────────────────┐
   * │ ĐỀ BÀI:                                                                     │
   * │ Đếm số bit 1 trong biểu diễn binary của n (Hamming Weight)                  │
   * ├─────────────────────────────────────────────────────────────────────────────┤
   * │                                                                             │
   * │ VÍ DỤ: 11 = 1011 → Output: 3 (có 3 bit 1)                                   │
   * │                                                                             │
   * │ CÁCH 1: Check từng bit (32 lần)                                             │
   * │ while (n != 0) { if ((n & 1) == 1) count++; n >>>= 1; }                     │
   * │                                                                             │
   * │ CÁCH 2: n & (n-1) trick ⭐ (tối ưu hơn)                                     │
   * │ n & (n-1) XÓA bit 1 thấp nhất!                                              │
   * │                                                                             │
   * │ TẠI SAO n & (n-1) xóa bit 1 thấp nhất?                                      │
   * │ ─────────────────────────────────────                                       │
   * │ n     = 1010 (có bit 1 ở vị trí 1 là thấp nhất)                             │
   * │ n-1   = 1001 (trừ 1: bit 1 thấp nhất → 0, các bit 0 bên phải → 1)           │
   * │ n&n-1 = 1000 (AND: bit 1 thấp nhất bị xóa!)                                 │
   * │                                                                             │
   * │ n     = 1100                                                                │
   * │ n-1   = 1011                                                                │
   * │ n&n-1 = 1000                                                                │
   * │                                                                             │
   * │ → Đếm số lần thực hiện n & (n-1) đến khi n = 0 = số bit 1                   │
   * └─────────────────────────────────────────────────────────────────────────────┘
   */
  public int hammingWeight(int n) {
    int count = 0;
    while (n != 0) {
      n &= (n - 1);  // Xóa bit 1 thấp nhất
      count++;       // Đếm
    }
    return count;
  }

  @Test
  void testHammingWeight() {
    assertEquals(3, hammingWeight(0b1011));       // 11
    assertEquals(1, hammingWeight(0b10000000));   // 128
    assertEquals(31, hammingWeight(Integer.MAX_VALUE)); // 2^31 - 1
  }

  // ═══════════════════════════════════════════════════════════════════════════
  // #3. LC 231 - POWER OF TWO (Easy)
  // ═══════════════════════════════════════════════════════════════════════════
  /**
   * ┌─────────────────────────────────────────────────────────────────────────────┐
   * │ ĐỀ BÀI:                                                                     │
   * │ Kiểm tra n có phải lũy thừa của 2 không (1, 2, 4, 8, 16, ...)               │
   * ├─────────────────────────────────────────────────────────────────────────────┤
   * │                                                                             │
   * │ KEY INSIGHT:                                                                │
   * │ Power of 2 chỉ có ĐÚNG 1 bit = 1:                                           │
   * │   1 = 0001                                                                  │
   * │   2 = 0010                                                                  │
   * │   4 = 0100                                                                  │
   * │   8 = 1000                                                                  │
   * │                                                                             │
   * │ n & (n-1) xóa bit 1 thấp nhất:                                              │
   * │ • Nếu n là power of 2 (chỉ có 1 bit 1) → n & (n-1) = 0                      │
   * │ • Nếu n không phải power of 2 (>1 bit 1) → n & (n-1) ≠ 0                    │
   * │                                                                             │
   * │ VÍ DỤ:                                                                      │
   * │   8 = 1000, 8-1 = 0111, 8 & 7 = 0 → ✓ Power of 2                            │
   * │   6 = 0110, 6-1 = 0101, 6 & 5 = 0100 ≠ 0 → ✗ Không phải                     │
   * └─────────────────────────────────────────────────────────────────────────────┘
   */
  public boolean isPowerOfTwo(int n) {
    // n > 0: loại số âm và 0
    // n & (n-1) == 0: chỉ có 1 bit = 1
    return n > 0 && (n & (n - 1)) == 0;
  }

  @Test
  void testIsPowerOfTwo() {
    assertTrue(isPowerOfTwo(1));    // 2^0
    assertTrue(isPowerOfTwo(16));   // 2^4
    assertTrue(isPowerOfTwo(1024)); // 2^10
    assertFalse(isPowerOfTwo(3));
    assertFalse(isPowerOfTwo(0));
    assertFalse(isPowerOfTwo(-2));

    System.out.println(Integer.toBinaryString(19));
    System.out.println(Integer.toBinaryString(20));
  }

  // ═══════════════════════════════════════════════════════════════════════════
  // #4. LC 268 - MISSING NUMBER (Easy)
  // ═══════════════════════════════════════════════════════════════════════════
  /**
   * ┌─────────────────────────────────────────────────────────────────────────────┐
   * │ ĐỀ BÀI:                                                                     │
   * │ Mảng chứa n số từ 0 đến n, thiếu 1 số. Tìm số đó.                           │
   * ├─────────────────────────────────────────────────────────────────────────────┤
   * │                                                                             │
   * │ VÍ DỤ: [3, 0, 1] (n=3, dãy 0-3) → Output: 2 (thiếu số 2)                    │
   * │                                                                             │
   * │ CÁCH GIẢI:                                                                  │
   * │ • XOR tất cả các số từ 0 đến n                                              │
   * │ • XOR với tất cả các số trong mảng                                          │
   * │ • Các số trùng nhau triệt tiêu, còn lại số thiếu!                           │
   * │                                                                             │
   * │ MINH HỌA: [3, 0, 1] (n = 3)                                                 │
   * │                                                                             │
   * │ XOR indices: 0 ^ 1 ^ 2 ^ 3 (các index từ 0 đến n)                           │
   * │ XOR values:  3 ^ 0 ^ 1     (các giá trị trong mảng)                         │
   * │                                                                             │
   * │ Kết hợp: 0 ^ 1 ^ 2 ^ 3 ^ 3 ^ 0 ^ 1                                          │
   * │        = (0^0) ^ (1^1) ^ (3^3) ^ 2                                          │
   * │        = 0 ^ 0 ^ 0 ^ 2                                                      │
   * │        = 2 ← Số thiếu!                                                      │
   * └─────────────────────────────────────────────────────────────────────────────┘
   */
  public int missingNumber(int[] nums) {
    int n = nums.length;
    int xor = n;  // Bắt đầu với n (vì index chỉ từ 0 đến n-1)

    for (int i = 0; i < n; i++) {
      xor ^= i;        // XOR index
      xor ^= nums[i];  // XOR value
    }

    return xor;
  }

  @Test
  void testMissingNumber() {
    assertEquals(2, missingNumber(new int[]{3, 0, 1}));
    assertEquals(2, missingNumber(new int[]{0, 1}));
    assertEquals(8, missingNumber(new int[]{9, 6, 4, 2, 3, 5, 7, 0, 1}));
    assertEquals(0, missingNumber(new int[]{1}));
  }

  // ═══════════════════════════════════════════════════════════════════════════
  // #5. LC 190 - REVERSE BITS (Easy)
  // ═══════════════════════════════════════════════════════════════════════════
  /**
   * ┌─────────────────────────────────────────────────────────────────────────────┐
   * │ ĐỀ BÀI:                                                                     │
   * │ Đảo ngược thứ tự 32 bits của số n                                           │
   * ├─────────────────────────────────────────────────────────────────────────────┤
   * │                                                                             │
   * │ VÍ DỤ:                                                                      │
   * │ n = 00000010100101000001111010011100                                        │
   * │   → 00111001011110000010100101000000                                        │
   * │                                                                             │
   * │ CÁCH GIẢI: Duyệt 32 bits, lấy từng bit và đặt vào vị trí đối xứng           │
   * │                                                                             │
   * │ THUẬT TOÁN:                                                                 │
   * │ for i = 0 to 31:                                                            │
   * │   1. Lấy bit thứ i của n:     bit = (n >> i) & 1                            │
   * │   2. Đặt vào vị trí 31-i:     result |= (bit << (31 - i))                   │
   * │                                                                             │
   * │ MINH HỌA với 4 bits (đơn giản hơn):                                         │
   * │ n = 1011 (đảo thành 1101)                                                   │
   * │                                                                             │
   * │ i=0: bit = 1, đặt vào pos 3 → result = 1000                                 │
   * │ i=1: bit = 1, đặt vào pos 2 → result = 1100                                 │
   * │ i=2: bit = 0, đặt vào pos 1 → result = 1100 (không đổi)                     │
   * │ i=3: bit = 1, đặt vào pos 0 → result = 1101                                 │
   * └─────────────────────────────────────────────────────────────────────────────┘
   */
  public int reverseBits(int n) {
    int result = 0;
    for (int i = 0; i < 32; i++) {
      // Bước 1: Lấy bit thứ i của n
      int bit = (n >> i) & 1;

      // Bước 2: Đặt bit đó vào vị trí (31 - i) trong result
      result |= (bit << (31 - i));
    }
    return result;
  }

  @Test
  void testReverseBits() {
    assertEquals(964176192, reverseBits(43261596));
    assertEquals(0, reverseBits(0));
    // 1 ở bit 0 → đảo → 1 ở bit 31
    assertEquals(Integer.MIN_VALUE, reverseBits(1));
  }

  // ═══════════════════════════════════════════════════════════════════════════
  // CÁCH 2: DÙNG JAVA BUILT-IN (Ngắn nhất)
  // ═══════════════════════════════════════════════════════════════════════════
  public int reverseBitsJava(int n) {
    return Integer.reverse(n);
  }

  // ═══════════════════════════════════════════════════════════════════════════
  // CÁCH 3: DIVIDE AND CONQUER (Bit Manipulation Magic - Không dùng loop)
  // ═══════════════════════════════════════════════════════════════════════════
  public int reverseBitsOpt(int n) {
    // 1. Swap adjacent bits (đổi chỗ 2 bit liền kề)
    // 0x55555555 = 01010101... (mask lẻ)
    // 0xAAAAAAAA = 10101010... (mask chẵn)
    n = ((n & 0x55555555) << 1) | ((n >>> 1) & 0x55555555);

    // 2. Swap 2-bit pairs (đổi chỗ cặp 2 bit)
    // 0x33333333 = 00110011...
    n = ((n & 0x33333333) << 2) | ((n >>> 2) & 0x33333333);

    // 3. Swap nibbles (đổi chỗ cặp 4 bit)
    // 0x0F0F0F0F = 00001111...
    n = ((n & 0x0F0F0F0F) << 4) | ((n >>> 4) & 0x0F0F0F0F);

    // 4. Swap bytes (đổi chỗ cặp 8 bit)
    // 0x00FF00FF = 0000000011111111...
    n = ((n & 0x00FF00FF) << 8) | ((n >>> 8) & 0x00FF00FF);

    // 5. Swap halves (đổi chỗ 16 bit đầu và cuối)
    // 0x0000FFFF (thực ra chỉ cần dịch là đủ vì int 32 bit)
    n = (n << 16) | (n >>> 16);

    return n;
  }


  // ═══════════════════════════════════════════════════════════════════════════
  // #6. LC 338 - COUNTING BITS (Easy)
  // ═══════════════════════════════════════════════════════════════════════════
  /**
   * ┌─────────────────────────────────────────────────────────────────────────────┐
   * │ ĐỀ BÀI:                                                                     │
   * │ Với mỗi số từ 0 đến n, đếm số bit 1. Trả về mảng kết quả.                   │
   * ├─────────────────────────────────────────────────────────────────────────────┤
   * │                                                                             │
   * │ VÍ DỤ: n = 5 → [0, 1, 1, 2, 1, 2]                                           │
   * │   0 = 0000 → 0 bit 1                                                        │
   * │   1 = 0001 → 1 bit 1                                                        │
   * │   2 = 0010 → 1 bit 1                                                        │
   * │   3 = 0011 → 2 bit 1                                                        │
   * │   4 = 0100 → 1 bit 1                                                        │
   * │   5 = 0101 → 2 bit 1                                                        │
   * │                                                                             │
   * │ CÁCH 1: Dùng hammingWeight cho từng số - O(n * 32)                          │
   * │                                                                             │
   * │ CÁCH 2: DP với n & (n-1) - O(n) ⭐                                           │
   * │ dp[i] = dp[i & (i-1)] + 1                                                   │
   * │                                                                             │
   * │ Giải thích: i & (i-1) xóa bit 1 thấp nhất                                   │
   * │ → Số bit 1 của i = số bit 1 của (i với bit 1 thấp nhất bị xóa) + 1          │
   * │                                                                             │
   * │ VD: i = 5 = 101                                                             │
   * │     i & (i-1) = 5 & 4 = 101 & 100 = 100 = 4                                 │
   * │     dp[5] = dp[4] + 1 = 1 + 1 = 2 ✓                                         │
   * └─────────────────────────────────────────────────────────────────────────────┘
   */
  public int[] countBits(int n) {
    int[] dp = new int[n + 1];
    // dp[0] = 0 (số 0 có 0 bit 1)

    for (int i = 1; i <= n; i++) {
      // i & (i-1) xóa bit 1 thấp nhất
      // Kết quả là số đã tính trước đó + 1
      dp[i] = dp[i & (i - 1)] + 1;
    }

    return dp;
  }

  @Test
  void testCountBits() {
    assertArrayEquals(new int[]{0, 1, 1}, countBits(2));
    assertArrayEquals(new int[]{0, 1, 1, 2, 1, 2}, countBits(5));
  }

  // ═══════════════════════════════════════════════════════════════════════════
  // #7. LC 461 - HAMMING DISTANCE (Easy)
  // ═══════════════════════════════════════════════════════════════════════════
  /**
   * ┌─────────────────────────────────────────────────────────────────────────────┐
   * │ ĐỀ BÀI:                                                                     │
   * │ Tính Hamming Distance giữa 2 số = số bit khác nhau                          │
   * ├─────────────────────────────────────────────────────────────────────────────┤
   * │                                                                             │
   * │ VÍ DỤ: x = 1 (0001), y = 4 (0100) → Output: 2                               │
   * │   0001                                                                      │
   * │   0100                                                                      │
   * │   ────                                                                      │
   * │   bit 0 khác (0 vs 1)                                                       │
   * │   bit 2 khác (1 vs 0)                                                       │
   * │   → Hamming Distance = 2                                                    │
   * │                                                                             │
   * │ CÁCH GIẢI:                                                                  │
   * │ 1. XOR 2 số: bit khác nhau → 1, giống nhau → 0                              │
   * │ 2. Đếm số bit 1 trong kết quả XOR                                           │
   * │                                                                             │
   * │ VD: 1 ^ 4 = 0001 ^ 0100 = 0101 → 2 bit 1 → Distance = 2                     │
   * └─────────────────────────────────────────────────────────────────────────────┘
   */
  public int hammingDistance(int x, int y) {
    int xor = x ^ y;  // Bit khác nhau = 1
    return hammingWeight(xor);  // Đếm số bit 1
  }

  @Test
  void testHammingDistance() {
    assertEquals(2, hammingDistance(1, 4));
    assertEquals(0, hammingDistance(5, 5));
    assertEquals(1, hammingDistance(3, 1));  // 11 vs 01
  }

  // ═══════════════════════════════════════════════════════════════════════════
  // #8. LC 78 - SUBSETS (Medium) ⭐ QUAN TRỌNG
  // ═══════════════════════════════════════════════════════════════════════════
  /**
   * ┌─────────────────────────────────────────────────────────────────────────────┐
   * │ ĐỀ BÀI:                                                                     │
   * │ Cho mảng các số unique, trả về tất cả subsets (power set)                   │
   * ├─────────────────────────────────────────────────────────────────────────────┤
   * │                                                                             │
   * │ VÍ DỤ: [1, 2, 3] → [[], [1], [2], [3], [1,2], [1,3], [2,3], [1,2,3]]        │
   * │                                                                             │
   * │ KEY INSIGHT: Dùng bitmask để biểu diễn subset                               │
   * │ ────────────────────────────────────────────                                │
   * │ n phần tử → 2^n subsets                                                     │
   * │ Mỗi số từ 0 đến 2^n - 1 đại diện cho 1 subset                               │
   * │ Bit thứ i = 1 → bao gồm phần tử thứ i                                       │
   * │                                                                             │
   * │ VD với [1, 2, 3]:                                                           │
   * │   mask = 0 = 000 → []           (không chọn gì)                             │
   * │   mask = 1 = 001 → [1]          (chọn phần tử 0)                            │
   * │   mask = 2 = 010 → [2]          (chọn phần tử 1)                            │
   * │   mask = 3 = 011 → [1, 2]       (chọn phần tử 0 và 1)                       │
   * │   mask = 4 = 100 → [3]          (chọn phần tử 2)                            │
   * │   mask = 5 = 101 → [1, 3]       (chọn phần tử 0 và 2)                       │
   * │   mask = 6 = 110 → [2, 3]       (chọn phần tử 1 và 2)                       │
   * │   mask = 7 = 111 → [1, 2, 3]    (chọn tất cả)                               │
   * └─────────────────────────────────────────────────────────────────────────────┘
   */
  public List<List<Integer>> subsets(int[] nums) {
    List<List<Integer>> result = new ArrayList<>();
    int n = nums.length;
    int totalSubsets = 1 << n;  // 2^n

    // Duyệt từ 0 đến 2^n - 1
    for (int mask = 0; mask < totalSubsets; mask++) {
      List<Integer> subset = new ArrayList<>();

      // Check từng bit của mask
      for (int i = 0; i < n; i++) {
        // Nếu bit thứ i = 1 → thêm phần tử thứ i
        if ((mask & (1 << i)) != 0) {
          subset.add(nums[i]);
        }
      }

      result.add(subset);
    }

    return result;
  }

  @Test
  void testSubsets() {
    List<List<Integer>> result = subsets(new int[]{1, 2, 3});
    assertEquals(8, result.size());  // 2^3 = 8 subsets
  }

  // ═══════════════════════════════════════════════════════════════════════════
  // #9. LC 260 - SINGLE NUMBER III (Medium) ⭐ ADVANCED
  // ═══════════════════════════════════════════════════════════════════════════
  /**
   * ┌─────────────────────────────────────────────────────────────────────────────┐
   * │ ĐỀ BÀI:                                                                     │
   * │ Mảng có các số xuất hiện 2 lần, CHỈ 2 số xuất hiện 1 lần.                   │
   * │ Tìm 2 số đó.                                                                │
   * ├─────────────────────────────────────────────────────────────────────────────┤
   * │                                                                             │
   * │ VÍ DỤ: [1, 2, 1, 3, 2, 5] → Output: [3, 5]                                  │
   * │                                                                             │
   * │ 💡 XOR tất cả cho ra x ^ y, nhưng làm sao tách x và y?                      │
   * │                                                                             │
   * │ THUẬT TOÁN:                                                                 │
   * │ ─────────────────────────────────────────                                   │
   * │ B1: XOR tất cả → xorResult = x ^ y                                          │
   * │     VD: 1^2^1^3^2^5 = 3^5 = 011^101 = 110 = 6                               │
   * │                                                                             │
   * │ B2: Tìm bit khác nhau giữa x và y (bất kỳ bit 1 nào trong xorResult)        │
   * │     Dùng: diffBit = xorResult & (-xorResult) = bit 1 thấp nhất              │
   * │     VD: 6 & (-6) = 110 & 010 = 010 = 2 (bit 1 ở vị trí 1)                   │
   * │                                                                             │
   * │ B3: Chia mảng thành 2 nhóm dựa trên diffBit, XOR mỗi nhóm                   │
   * │     - Nhóm 1: các số có diffBit = 0                                         │
   * │     - Nhóm 2: các số có diffBit = 1                                         │
   * │     x và y chắc chắn ở 2 nhóm khác nhau (vì bit đó khác nhau)!              │
   * │                                                                             │
   * │ MINH HỌA với [1, 2, 1, 3, 2, 5], diffBit = 2 (bit 1):                       │
   * │                                                                             │
   * │ Nhóm có bit 1 = 0: 1(01), 1(01), 5(101) → XOR = 01^01^101 = 101 = 5         │
   * │ Nhóm có bit 1 = 1: 2(10), 3(11), 2(10) → XOR = 10^11^10 = 11 = 3            │
   * │                                                                             │
   * │ → x = 3, y = 5 ✓                                                            │
   * └─────────────────────────────────────────────────────────────────────────────┘
   */
  public int[] singleNumberIII(int[] nums) {
    // B1: XOR tất cả → x ^ y
    int xorResult = 0;
    for (int num : nums) {
      xorResult ^= num;
    }

    // B2: Tìm bit khác nhau (bit 1 thấp nhất)
    int diffBit = xorResult & (-xorResult);

    // B3: Chia 2 nhóm và XOR
    int x = 0, y = 0;
    for (int num : nums) {
      if ((num & diffBit) == 0) {
        x ^= num;  // Nhóm có diffBit = 0
      } else {
        y ^= num;  // Nhóm có diffBit = 1
      }
    }

    return new int[]{x, y};
  }

  @Test
  void testSingleNumberIII() {
    int[] result = singleNumberIII(new int[]{1, 2, 1, 3, 2, 5});
    Arrays.sort(result);
    assertArrayEquals(new int[]{3, 5}, result);

    result = singleNumberIII(new int[]{0, 1});
    Arrays.sort(result);
    assertArrayEquals(new int[]{0, 1}, result);
  }

  // ═══════════════════════════════════════════════════════════════════════════
  // BONUS: LC 371 - SUM OF TWO INTEGERS (Medium)
  // ═══════════════════════════════════════════════════════════════════════════
  /**
   * ┌─────────────────────────────────────────────────────────────────────────────┐
   * │ ĐỀ BÀI:                                                                     │
   * │ Tính a + b mà KHÔNG dùng + hoặc -                                           │
   * ├─────────────────────────────────────────────────────────────────────────────┤
   * │                                                                             │
   * │ KEY INSIGHT: Mô phỏng phép cộng binary                                      │
   * │                                                                             │
   * │ Khi cộng 2 bit:                                                             │
   * │   0 + 0 = 0, carry = 0                                                      │
   * │   0 + 1 = 1, carry = 0                                                      │
   * │   1 + 0 = 1, carry = 0                                                      │
   * │   1 + 1 = 0, carry = 1  ← XOR cho sum, AND cho carry!                       │
   * │                                                                             │
   * │ THUẬT TOÁN:                                                                 │
   * │ 1. sum without carry = a XOR b                                              │
   * │ 2. carry = (a AND b) << 1  (shift left vì carry sang bit kế)                │
   * │ 3. Lặp lại: a = sum, b = carry, cho đến khi carry = 0                       │
   * │                                                                             │
   * │ VD: 5 + 3                                                                   │
   * │   5 = 101, 3 = 011                                                          │
   * │   sum = 101 ^ 011 = 110 = 6                                                 │
   * │   carry = (101 & 011) << 1 = 001 << 1 = 010 = 2                             │
   * │                                                                             │
   * │   Tiếp: a=6, b=2                                                            │
   * │   sum = 110 ^ 010 = 100 = 4                                                 │
   * │   carry = (110 & 010) << 1 = 010 << 1 = 100 = 4                             │
   * │                                                                             │
   * │   Tiếp: a=4, b=4                                                            │
   * │   sum = 100 ^ 100 = 000 = 0                                                 │
   * │   carry = (100 & 100) << 1 = 100 << 1 = 1000 = 8                            │
   * │                                                                             │
   * │   Tiếp: a=0, b=8                                                            │
   * │   sum = 000 ^ 1000 = 1000 = 8                                               │
   * │   carry = 0 → DONE!                                                         │
   * │                                                                             │
   * │   Result = 8 ✓ (5 + 3 = 8)                                                  │
   * └─────────────────────────────────────────────────────────────────────────────┘
   */
  public int getSum(int a, int b) {
    while (b != 0) {
      int carry = (a & b) << 1;  // Carry cần shift left
      a = a ^ b;                  // Sum without carry
      b = carry;                  // Tiếp tục với carry
    }
    return a;
  }

  @Test
  void testGetSum() {
    assertEquals(3, getSum(1, 2));
    assertEquals(5, getSum(2, 3));
    assertEquals(8, getSum(5, 3));
    assertEquals(0, getSum(-1, 1));
  }
  // ═══════════════════════════════════════════════════════════════════════════
  // #10. LC 3315 - Construct the Minimum Bitwise Array II (Medium)
  // ═══════════════════════════════════════════════════════════════════════════
  /**
   * ┌─────────────────────────────────────────────────────────────────────────────┐
   * │ ĐỀ BÀI:                                                                     │
   * │ Cho mảng nums chứa các số nguyên tố.                                        │
   * │ Tìm mảng ans sao cho ans[i] OR (ans[i] + 1) == nums[i].                     │
   * │ ans[i] phải nhỏ nhất có thể. Nếu không tìm được, ans[i] = -1.               │
   * ├─────────────────────────────────────────────────────────────────────────────┤
   * │                                                                             │
   * │ PHÂN TÍCH:                                                                  │
   * │ x | (x + 1) luôn có dạng ...011...1 OR ...100...0 = ...111...1              │
   * │ (Tức là bit 0 đầu tiên từ phải sang của x sẽ biến thành 1, và các bit 1     │
   * │ phía sau nó giữ nguyên).                                                    │
   * │                                                                             │
   * │ Suy ra: nums[i] phải là kết quả của việc "lấp đầy" chuỗi 1 liên tiếp        │
   * │ tính từ bit thấp nhất.                                                      │
   * │                                                                             │
   * │ Gọi L là độ dài chuỗi bit 1 liên tiếp ở cuối của nums[i].                   │
   * │ (Ví dụ: 7 = 111 → L=3, 13 = 1101 → L=1).                                    │
   * │                                                                             │
   * │ Để x nhỏ nhất, ta cần giữ lại nhiều bit cao nhất có thể của nums[i],        │
   * │ và xóa đi 1 bit trong chuỗi L bit 1 cuối cùng đó.                           │
   * │ Để x min, ta xóa bit CAO NHẤT trong chuỗi L bit 1 này (bit thứ L-1).        │
   * │                                                                             │
   * │ CÔNG THỨC:                                                                  │
   * │ Nếu nums[i] = 2 (10) → L=0 (trailing zeros, thực ra là tận cùng 0) → -1     │
   * │ (Vì primes trừ 2 ra đều lẻ, nên tận cùng luôn là 1).                        │
   * │                                                                             │
   * │ Với nums[i] lẻ: tìm bit 0 đầu tiên từ phải sang (ở vị trí L).               │
   * │ Bit cần xóa là L-1.                                                         │
   * │ ans[i] = nums[i] ^ (1 << (L-1))                                             │
   * │                                                                             │
   * │ THUẬT TOÁN:                                                                 │
   * │ 1. Nếu nu == 2 → ans = -1                                                   │
   * │ 2. Tìm bit 0 đầu tiên của nums[i] (tương đương bit 1 thấp nhất của ~nums[i])│
   * │    Hoặc đơn giản: tìm lowest set bit của (nums[i] + 1).                     │
   * │    Vd: 7 (0111) + 1 = 8 (1000) → bit 1 ở pos 3 (L=3).                       │
   * │ 3. ans = nums[i] - (bit >> 1)                                               │
   * │                                                                             │
   * │ VD: nums[i] = 7 (111). num+1 = 8 (1000). bit=8. sub=4. ans = 3 (011).       │
   * │     3 | 4 = 7. Correct.                                                     │
   * └─────────────────────────────────────────────────────────────────────────────┘
   */
  public int[] minBitwiseArray(List<Integer> nums) {
    int n = nums.size();
    int[] ans = new int[n];

    for (int i = 0; i < n; i++) {
      int val = nums.get(i);
      if (val == 2) {
        ans[i] = -1;
      } else {
        // Tìm bit 1 thấp nhất của (val + 1)
        // val là số lẻ (prime > 2), nên val luôn tận cùng là 1.
        // val + 1 sẽ tận cùng là 0...0
        // (val + 1) & -(val + 1) sẽ lấy ra bit 1 thấp nhất này.
        // Bit này chính là 1 << L.
        // Ta cần trừ đi 1 << (L-1), tức là bit này dịch phải 1.

        int lowestBitOfNext = (val + 1) & -(val + 1);
        ans[i] = val - (lowestBitOfNext >> 1);
      }
    }
    return ans;
  }

  @Test
  void testMinBitwiseArray() {
    // Test case 1: [2, 3, 5, 7]
    // 2 -> -1
    // 3 (011) -> L=2, sub 2^(2-1)=2. 3-2=1. Check 1|2=3. Correct.
    // 5 (101) -> L=1, sub 2^(1-1)=1. 5-1=4. Check 4|5=5. Correct.
    // 7 (111) -> L=3, sub 2^(3-1)=4. 7-4=3. Check 3|4=7. Correct.
    
    int[] expected = {-1, 1, 4, 3};
    int[] result = minBitwiseArray(Arrays.asList(2, 3, 5, 7));
    assertArrayEquals(expected, result);

    // Test case from description analysis: 13 (1101)
    // 13 -> L=1. ans = 12. 12|13=13.
    assertArrayEquals(new int[]{12}, minBitwiseArray(Arrays.asList(13)));
  }

  // ═══════════════════════════════════════════════════════════════════════════
  // #11. LC 29 - DIVIDE TWO INTEGERS (Medium)
  // ═══════════════════════════════════════════════════════════════════════════
  /**
   * ┌─────────────────────────────────────────────────────────────────────────────┐
   * │ ĐỀ BÀI:                                                                     │
   * │ Chia 2 số nguyên mà KHÔNG dùng phép nhân (*), chia (/), hoặc mod (%).       │
   * │ Trả về thương số (quotient). Truncate toward 0 (8.3 -> 8, -2.7 -> -2).      │
   * │ Xử lý overflow: [−2^31, 2^31 − 1]. Nếu tràn, trả về MAX_VALUE.              │
   * ├─────────────────────────────────────────────────────────────────────────────┤
   * │                                                                             │
   * │ Ý TƯỞNG CỐT LÕI: Phép chia thực chất là PHÉP TRỪ LẶP LẠI.                   │
   * │ 10 / 3 = 3 dư 1                                                             │
   * │ -> 10 - 3 = 7                                                               │
   * │ -> 7 - 3 = 4                                                                │
   * │ -> 4 - 3 = 1  (nhỏ hơn 3, dừng)                                             │
   * │ -> Trừ được 3 lần => kết quả là 3.                                          │
   * │                                                                             │
   * │ Vấn đề: Nếu 2^31 / 1 mà trừ từng cái 1 thì TLE (Time Limit Exceeded).       │
   * │ Giải pháp: TRỪ THEO CẤP SỐ NHÂN 2 (Binary Search / Bitwise Shift)           │
   * │                                                                             │
   * │ CƠ CHẾ "LŨY THỪA 2":                                                        │
   * │ Thay vì trừ divisor (3), ta thử trừ 3*2^10, 3*2^9, ..., 3*2^0.              │
   * │                                                                             │
   * │ VD: 100 / 3                                                                 │
   * │  3 * 2^0 = 3                                                                │
   * │  3 * 2^1 = 6                                                                │
   * │  3 * 2^2 = 12                                                               │
   * │  3 * 2^3 = 24                                                               │
   * │  3 * 2^4 = 48                                                               │
   * │  3 * 2^5 = 96  (nhỏ hơn 100)                                                │
   * │  3 * 2^6 = 192 (lớn hơn 100) -> Dừng tăng.                                  │
   * │                                                                             │
   * │ Bước 1: 100 - (3 * 2^5) = 100 - 96 = 4.  (Kết quả += 2^5 = 32)              │
   * │ Bước 2: 4 (còn lại) / 3.                                                    │
   * │         3 * 2^0 = 3 (nhỏ hơn 4).                                            │
   * │         4 - 3 = 1.                       (Kết quả += 2^0 = 1)               │
   * │ Tổng: 32 + 1 = 33.                                                          │
   * │                                                                             │
   * │ CÁCH LÀM:                                                                   │
   * │ 1. Xử lý dấu: quotient dương nếu dividend và divisor cùng dấu.                │
   * │ 2. Chuyển dividend và divisor sang số dương (nên dùng long để tránh tràn).    │
   * │ 3. Dùng bit shift (<<) để nhân 2.                                           │
   * │                                                                             │
   * └─────────────────────────────────────────────────────────────────────────────┘
   */
  public int divide(int dividend, int divisor) {
    // 1. Xử lý trường hợp đặc biệt: Overflow
    // Chỉ có 1 trường hợp tràn số: MIN_VALUE / -1 = MAX_VALUE + 1 (quá int)
    if (dividend == Integer.MIN_VALUE && divisor == -1) {
      return Integer.MAX_VALUE;
    }

    // 2. Xác định dấu của kết quả
    // Kết quả âm nếu 2 số trái dấu (XOR < 0)
    boolean negative = (dividend < 0) ^ (divisor < 0);

    // 3. Chuyển sang giá trị tuyệt đối (dùng long để tránh tràn khi abs MIN_VALUE)
    long absDividend = Math.abs((long) dividend);
    long absDivisor = Math.abs((long) divisor);

    int result = 0;

    // 4. Lặp để trừ dần
    // Trong khi số bị chia vẫn lớn hơn số chia
    while (absDividend >= absDivisor) {
      long tempDivisor = absDivisor;
      long multiple = 1;

      // Tìm bội số lớn nhất của divisor mà <= dividend
      // (nhân đôi divisor liên tục)
      // tempDivisor << 1 chính là tempDivisor * 2
      while (absDividend >= (tempDivisor << 1)) {
        tempDivisor <<= 1;
        multiple <<= 1;
      }

      // Trừ phần đó ra khỏi số bị chia
      absDividend -= tempDivisor;
      // Cộng số lần trừ vào kết quả
      result += multiple;
    }

    // 5. Trả về kết quả kèm dấu
    return negative ? -result : result;
  }

  @Test
  void testDivide() {
    assertEquals(3, divide(10, 3));
    assertEquals(-2, divide(7, -3));
    assertEquals(0, divide(0, 1));
    assertEquals(1, divide(1, 1));
    assertEquals(Integer.MAX_VALUE, divide(Integer.MIN_VALUE, -1)); // Overflow case
    assertEquals(Integer.MIN_VALUE, divide(Integer.MIN_VALUE, 1));
  }

  // ═══════════════════════════════════════════════════════════════════════════
  // #12. LC 67 - ADD BINARY (Easy)
  // ═══════════════════════════════════════════════════════════════════════════
  /**
   * ┌─────────────────────────────────────────────────────────────────────────────┐
   * │ ĐỀ BÀI:                                                                     │
   * │ Cộng 2 chuỗi nhị phân (binary strings).                                     │
   * │ Input: a = "11", b = "1" -> Output: "100"                                   │
   * ├─────────────────────────────────────────────────────────────────────────────┤
   * │                                                                             │
   * │ Ý TƯỞNG: Mô phỏng phép cộng tiểu học (cộng từng cột từ phải sang trái).     │
   * │                                                                             │
   * │    1 1   (a)                                                                │
   * │ +    1   (b)                                                                │
   * │ ──────                                                                      │
   * │                                                                             │
   * │ B1: Cột ngoài cùng: 1 + 1 = 2 -> viết 0 nhớ 1.                              │
   * │ B2: Cột tiếp theo:  1 + (nhớ 1) = 2 -> viết 0 nhớ 1.                        │
   * │ B3: Hết số -> Hạ 1 xuống.                                                   │
   * │ -> Kết quả: 100.                                                            │
   * │                                                                             │
   * │ THUẬT TOÁN (Two Pointers):                                                  │
   * │ 1. Dùng i và j trỏ vào cuối chuỗi a và b.                                   │
   * │ 2. Dùng biến carry để lưu số nhớ.                                           │
   * │ 3. Lặp khi (i >= 0 HOẶC j >= 0 HOẶC carry > 0).                             │
   * │    - sum = carry                                                            │
   * │    - Nếu còn i: sum += a[i], i--                                            │
   * │    - Nếu còn j: sum += b[j], j--                                            │
   * │    - carry mới = sum / 2                                                    │
   * │    - digit viết vào = sum % 2                                               │
   * │ 4. Kết quả bị ngược -> Đảo ngược lại (StringBuilder.reverse).               │
   * └─────────────────────────────────────────────────────────────────────────────┘
   */
  public String addBinary(String a, String b) {
    StringBuilder sb = new StringBuilder();
    int i = a.length() - 1;
    int j = b.length() - 1;
    int carry = 0;

    // Lặp đến khi hết cả 2 chuỗi VÀ hết số nhớ
    while (i >= 0 || j >= 0 || carry > 0) {
      int sum = carry;

      // Cộng bit từ chuỗi a (nếu còn)
      if (i >= 0) {
        sum += a.charAt(i) - '0'; // Chuyển char '0'/'1' sang int 0/1
        i--;
      }

      // Cộng bit từ chuỗi b (nếu còn)
      if (j >= 0) {
        sum += b.charAt(j) - '0';
        j--;
      }

      // sum có thể là:
      // 0 (0+0+0) -> viết 0, nhớ 0
      // 1 (0+1+0) -> viết 1, nhớ 0
      // 2 (1+1+0) -> viết 0, nhớ 1
      // 3 (1+1+1) -> viết 1, nhớ 1

      sb.append(sum % 2); // Viết phần đơn vị
      carry = sum / 2;    // Tính phần nhớ
    }

    return sb.reverse().toString();
  }

  @Test
  void testAddBinary() {
    assertEquals("100", addBinary("11", "1"));
    assertEquals("10101", addBinary("1010", "1011"));
    assertEquals("0", addBinary("0", "0"));
  }
}
