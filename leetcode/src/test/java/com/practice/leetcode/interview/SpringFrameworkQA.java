...
public class SpringFrameworkQA {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // COMMON SPRING DEVELOPER MISTAKES
  // ═══════════════════════════════════════════════════════════════════════════
  /**
   * ┌─────────────────────────────────────────────────────────────────────────────┐
   * │ 1. PRIVATE/PROTECTED METHODS VỚI ANNOTATIONS                                │
   * ├─────────────────────────────────────────────────────────────────────────────┤
   * │                                                                             │
   * │ ❌ SAI:                                                                     │
   * │ @Transactional                                                              │
   * │ private void saveUser() { }  // KHÔNG WORK!                                 │
   * │                                                                             │
   * │ @Cacheable("users")                                                         │
   * │ protected User getUser() { }  // KHÔNG WORK!                                │
   * │                                                                             │
   * │ @Async                                                                      │
   * │ private void processAsync() { }  // KHÔNG WORK!                             │
   * │                                                                             │
   * │ ✓ ĐÚNG:                                                                     │
   * │ @Transactional                                                              │
   * │ public void saveUser() { }  // Phải PUBLIC!                                 │
   * │                                                                             │
   * │ LÝ DO:                                                                      │
   * │ - Spring dùng PROXY để intercept method calls                               │
   * │ - CGLIB proxy = subclass → KHÔNG override được private                      │
   * │ - JDK proxy = interface → chỉ có public methods                             │
   * └─────────────────────────────────────────────────────────────────────────────┘
   *
   * ┌─────────────────────────────────────────────────────────────────────────────┐
   ...
   * │     public void process() {                                                 │
   * │         saveUser();  // Gọi trực tiếp, bypass proxy!                        │
   * │     }                                                                       │
   * │                                                                             │
   * │     @Transactional                                                          │
   * │     public void saveUser() { }  // KHÔNG có transaction!                    │
   * │ }                                                                           │
   * │                                                                             │
   * │ ✓ FIX 1: Inject self                                                        │
   * │ @Autowired                                                                  │
   * │ private UserService self;  // Hoặc dùng ApplicationContext                  │
   * │                                                                             │
   * │ public void process() {                                                     │
   * │     self.saveUser();  // Gọi qua proxy!                                     │
   * │ }                                                                           │
   * │                                                                             │
   * │ ✓ FIX 2: Tách ra service khác                                               │
   * │ userTransactionService.saveUser();                                          │
   * └─────────────────────────────────────────────────────────────────────────────┘
   *
   * ┌─────────────────────────────────────────────────────────────────────────────┐
   * │ 3. FINAL CLASS/METHOD VỚI CGLIB PROXY                                       │
   * ├─────────────────────────────────────────────────────────────────────────────┤
   * │                                                                             │
   * │ ❌ SAI:                                                                     │
   * │ @Service                                                                    │
   * │ public final class UserService { }  // CGLIB không thể extend!              │
   * │                                                                             │
   * │ @Transactional                                                              │
   * │ public final void save() { }  // CGLIB không thể override!                  │
   * │                                                                             │
   * │ ✓ ĐÚNG:                                                                     │
   * │ @Service                                                                    │
   * │ public class UserService { }  // Bỏ final                                   │
   * │                                                                             │
   * │ LÝ DO: CGLIB tạo subclass để proxy → final không extend được                │
   * └─────────────────────────────────────────────────────────────────────────────┘
   *
   * ┌─────────────────────────────────────────────────────────────────────────────┐
   ...
   * │ public void save() throws IOException { }                                   │
   * │                                                                             │
   * │ @Transactional(rollbackFor = IOException.class)  // Rollback cụ thể         │
   * │ public void save() throws IOException { }                                   │
   * │                                                                             │
   * │ DEFAULT: Chỉ rollback RuntimeException và Error                             │
   * └─────────────────────────────────────────────────────────────────────────────┘
   *
   * ┌─────────────────────────────────────────────────────────────────────────────┐
   * │ 5. CATCH EXCEPTION NUỐT MẤT ROLLBACK                                        │
   * ├─────────────────────────────────────────────────────────────────────────────┤
   * │                                                                             │
   * │ ❌ SAI:                                                                     │
   * │ @Transactional                                                              │
   ...
   * │     } catch (Exception e) {                                                 │
   * │         log.error("Error", e);  // Nuốt exception → KHÔNG ROLLBACK!         │
   * │     }                                                                       │
   * │ }                                                                           │
   * │                                                                             │
   * │ ✓ ĐÚNG:                                                                     │
   ...
   * │     } catch (Exception e) {                                                 │
   * │         log.error("Error", e);                                              │
   * │         throw e;  // Re-throw để rollback!                                  │
   * │     }                                                                       │
   * │ }                                                                           │
   * └─────────────────────────────────────────────────────────────────────────────┘
   *
   * ┌─────────────────────────────────────────────────────────────────────────────┐
   * │ 6. @Autowired TRÊN FIELD THAY VÌ CONSTRUCTOR                                │
   * ├─────────────────────────────────────────────────────────────────────────────┤
   * │                                                                             │
   * │ ❌ KHÔNG NÊN (khó test, hidden dependencies):                               │
   * │ @Service                                                                    │
   * │ public class UserService {                                                  │
   * │     @Autowired                                                              │
   * │     private UserRepository repo;                                            │
   ...
   * │ public class UserService {                                                  │
   * │     private final UserRepository repo;  // Auto-injected                    │
   * │ }                                                                           │
   * └─────────────────────────────────────────────────────────────────────────────┘
   *
   * ┌─────────────────────────────────────────────────────────────────────────────┐
   * │ 7. KHÔNG DÙNG INTERFACE CHO REPOSITORY/SERVICE                              │
   * ├─────────────────────────────────────────────────────────────────────────────┤
   * │                                                                             │
   * │ ❌ KHÔNG NÊN (tightly coupled):                                             │
   * │ @Autowired                                                                  │
   * │ private UserServiceImpl userService;  // Concrete class!                    │
   * │                                                                             │
   * │ ✓ NÊN (loosely coupled, mockable):                                          │
   * │ @Autowired                                                                  │
   * │ private UserService userService;  // Interface!                             │
   * │                                                                             │
   * │ LÝ DO:                                                                      │
   * │ - Easier testing (mock interface)                                           │
   * │ - JDK dynamic proxy works                                                   │
   * │ - Swap implementation easily                                                │
   * └─────────────────────────────────────────────────────────────────────────────┘
   *
   * ┌─────────────────────────────────────────────────────────────────────────────┐
   * │ 8. LAZY LOADING OUTSIDE TRANSACTION                                         │
   * ├─────────────────────────────────────────────────────────────────────────────┤
   * │                                                                             │
   * │ ❌ SAI:                                                                     │
   * │ @Transactional                                                              │
   * │ public User getUser(Long id) {                                              │
   * │     return userRepo.findById(id);  // TX ends after return                  │
   * │ }                                                                           │
   * │                                                                             │
   * │ // Caller:                                                                  │
   ...
   *
   * ┌─────────────────────────────────────────────────────────────────────────────┐
   * │ 9. MISSING @EnableXxx ANNOTATIONS                                           │
   * ├─────────────────────────────────────────────────────────────────────────────┤
   * │                                                                             │
   * │ ❌ SAI (annotation không work):                                             │
   * │ @Async                                                                      │
   * │ public void processAsync() { }  // Chạy sync!                               │
   * │                                                                             │
   * │ @Cacheable("users")                                                         │
   * │ public User getUser() { }  // Không cache!                                  │
   * │                                                                             │
   * │ @Scheduled(fixedRate = 1000)                                                │
   * │ public void scheduled() { }  // Không chạy!                                 │
   * │                                                                             │
   * │ ✓ ĐÚNG (phải enable):                                                       │
   * │ @Configuration                                                              │
   * │ @EnableAsync           // Cho @Async                                        │
   * │ @EnableCaching         // Cho @Cacheable                                    │
   * │ @EnableScheduling      // Cho @Scheduled                                    │
   * │ @EnableTransactionManagement  // Thường auto-enable rồi                     │
   * │ public class AppConfig { }                                                  │
   * └─────────────────────────────────────────────────────────────────────────────┘
   *
   * ┌─────────────────────────────────────────────────────────────────────────────┐
   * │ 10. @Transactional TRONG NEW THREAD                                         │
   * ├─────────────────────────────────────────────────────────────────────────────┤
   * │                                                                             │
   * │ ❌ SAI:                                                                     │
   * │ @Transactional                                                              │
   ...
   * │ @Async                                                                      │
   * │ @Transactional                                                              │
   * │ public void processInNewThread() {  // TX mới trong async thread            │
   * │     repo.save(entity);                                                      │
   * │ }                                                                           │
   * │                                                                             │
   * │ LÝ DO: Transaction context lưu trong ThreadLocal                            │
   * └─────────────────────────────────────────────────────────────────────────────┘
   *
   * ┌─────────────────────────────────────────────────────────────────────────────┐
   * │ 11. @Value KHÔNG WORK TRONG CONSTRUCTOR                                     │
   * ├─────────────────────────────────────────────────────────────────────────────┤
   * │                                                                             │
   * │ ❌ SAI:                                                                     │
   * │ @Component                                                                  │
   ...
   * │                                                                             │
   * │     public MyBean() {                                                       │
   * │         System.out.println(appName);  // NULL! Inject sau constructor       │
   * │     }                                                                       │
   * │ }                                                                           │
   * │                                                                             │
   ...
   * │ }                                                                           │
   * │                                                                             │
   * │ ✓ HOẶC dùng @PostConstruct:                                                 │
   * │ @PostConstruct                                                              │
   * │ public void init() {                                                        │
   * │     System.out.println(appName);  // OK! Sau khi inject                     │
   * │ }                                                                           │
   * └─────────────────────────────────────────────────────────────────────────────┘
   *
   * ┌─────────────────────────────────────────────────────────────────────────────┐
   * │ QUICK REFERENCE: ANNOTATION REQUIREMENTS                                    │
   * ├─────────────────────────────────────────────────────────────────────────────┤
   * │                                                                             │
   * │ ┌──────────────────┬─────────┬─────────────┬─────────────┬────────────────┐ │
   * │ │ Annotation       │ Public? │ No final?   │ External    │ @Enable?       │ │
   * │ │                  │         │             │ call?       │                │ │
   * │ ├──────────────────┼─────────┼─────────────┼─────────────┼────────────────┤ │
   * │ │ @Transactional   │ ✓ YES   │ ✓ YES       │ ✓ YES       │ (auto)         │ │
   * │ │ @Async           │ ✓ YES   │ ✓ YES       │ ✓ YES       │ @EnableAsync   │ │
   * │ │ @Cacheable       │ ✓ YES   │ ✓ YES       │ ✓ YES       │ @EnableCaching │ │
   * │ │ @Scheduled       │ ✓ YES   │ ✓ YES       │ N/A         │ @EnableSchedul │ │
   * │ │ @PreAuthorize    │ ✓ YES   │ ✓ YES       │ ✓ YES       │ @EnableMethod..│ │
   * │ └──────────────────┴─────────┴─────────────┴─────────────┴────────────────┘ │
   * │                                                                             │
   * │ Public?      = Method phải public                                           │
   * │ No final?    = Class/method không được final                                │
   * │ External?    = Phải gọi từ bên ngoài class (không self-invoke)              │
   * │ @Enable?     = Cần annotation enable                                        │
   * └─────────────────────────────────────────────────────────────────────────────┘
   */

  // ═══════════════════════════════════════════════════════════════════════════
  // NESTED TRANSACTIONS (Transaction lồng nhau)
  // ═══════════════════════════════════════════════════════════════════════════
  /**
   * ┌─────────────────────────────────────────────────────────────────────────────┐
   * │ TRANSACTION PROPAGATION - Khi TX gọi TX                                     │
   * ├─────────────────────────────────────────────────────────────────────────────┤
   * │                                                                             │
   * │ @Service                                                                    │
   * │ class OrderService {                                                        │
   ...
   * │         paymentService.processPayment();  // Gọi method có @Transactional   │
   * │     }                                                                       │
   * │ }                                                                           │
   * │                                                                             │
   * │ @Service                                                                    │
   ...
   * │ }                                                                           │
   * │                                                                             │
   * │ → Câu hỏi: processPayment() dùng TX nào?                                    │
   * │ → Trả lời: Phụ thuộc vào PROPAGATION setting!                               │
   * └─────────────────────────────────────────────────────────────────────────────┘
   *
   * ┌─────────────────────────────────────────────────────────────────────────────┐
   * │ PROPAGATION TYPES                                                           │
   * ├─────────────────────────────────────────────────────────────────────────────┤
   * │                                                                             │
   * │ 1. REQUIRED (default) - Dùng TX hiện có, hoặc tạo mới nếu không có          │
   * │ ─────────────────────────────────────────────────                           │
   * │ @Transactional(propagation = Propagation.REQUIRED)                          │
   * │                                                                             │
   * │ Có TX    → JOIN vào TX đó                                                   │
   * │ Không TX → Tạo TX mới                                                       │
   * │                                                                             │
   * │ ┌────────────────────────────────────────────────────────────┐              │
   * │ │ createOrder() [TX1]                                       │               │
   * │ │    │                                                       │              │
   * │ │    └──► processPayment() [JOIN TX1]  ← Cùng 1 TX!          │              │
   * │ │                                                            │              │
   * │ │ Nếu processPayment() throw → Cả 2 đều ROLLBACK             │              │
   * │ └────────────────────────────────────────────────────────────┘              │
   * │                                                                             │
   * │ 2. REQUIRES_NEW - LUÔN tạo TX mới, suspend TX cũ                            │
   * │ ─────────────────────────────────────────────────────────────               │
   ...
   * │ │ createOrder() [TX1]                                        │              │
   * │ │    │                                                       │              │
   * │ │    └──► processPayment() [TX2 - NEW!]                      │              │
   * │ │         │                                                  │              │
   * │ │         └─ TX2 commit độc lập                              │              │
   * │ │                                                            │              │
   * │ │ Nếu TX1 rollback → TX2 đã commit vẫn giữ!                  │              │
   * │ │ Nếu TX2 rollback → TX1 có thể catch và tiếp tục            │              │
   * │ └────────────────────────────────────────────────────────────┘              │
   * │                                                                             │
   * │ USE CASE: Audit log, notification - cần persist dù main TX fail             │
   * │                                                                             │
   * │ 3. NESTED - Tạo savepoint trong TX hiện có                                  │
   * │ ─────────────────────────────────────────────────────────────               │
   ...
   * │ │    ├──► SAVEPOINT                                          │              │
   * │ │    │                                                       │              │
   * │ │    └──► processPayment() [NESTED in TX1]                   │              │
   * │ │         │                                                  │              │
   * │ │         └─ Nếu fail → Rollback to SAVEPOINT                │              │
   * │ │            TX1 vẫn tiếp tục được!                          │              │
   * │ │                                                            │              │
   * │ │ Nếu TX1 rollback → processPayment cũng rollback            │              │
   * │ └────────────────────────────────────────────────────────────┘              │
   * │                                                                             │
   * │ LƯU Ý: Chỉ work với JDBC savepoints (không phải tất cả DB)                  │
   * │                                                                             │
   * │ 4. SUPPORTS - Dùng TX nếu có, không thì chạy không TX                       │
   * │ 5. NOT_SUPPORTED - Suspend TX hiện có, chạy không TX                        │
   * │ 6. MANDATORY - BẮT BUỘC phải có TX, nếu không → Exception                   │
   * │ 7. NEVER - BẮT BUỘC không có TX, nếu có → Exception                         │
   * └─────────────────────────────────────────────────────────────────────────────┘
   *
   * ┌─────────────────────────────────────────────────────────────────────────────┐
   * │ THỰC TẾ: Các scenario phổ biến                                              │
   * ├─────────────────────────────────────────────────────────────────────────────┤
   * │                                                                             │
   * │ SCENARIO 1: Inner method fail → Cả 2 rollback (REQUIRED)                    │
   * │ ─────────────────────────────────────────────────────────                   │
   * │ @Transactional                                                              │
   * │ void createOrder() {                                                        │
   * │     orderRepo.save(order);              // ✓ Saved                          │
   * │     paymentService.process();           // 💥 throws exception              │
   * │ }                                                                           │
   * │                                                                             │
   ...
   * │ }                                                                           │
   * │                                                                             │
   * │ KẾT QUẢ: order KHÔNG được save (rollback cả 2)                              │
   * │                                                                             │
   * │ SCENARIO 2: Inner fail nhưng outer catch (REQUIRED) - TRAP!                 │
   * │ ─────────────────────────────────────────────────────────────               │
   * │ @Transactional                                                              │
   * │ void createOrder() {                                                        │
   * │     orderRepo.save(order);                                                  │
   * │     try {                                                                   │
   * │         paymentService.process();  // 💥 throws                             │
   * │     } catch (Exception e) {                                                 │
   * │         log.error("Payment failed, but continue");                          │
   * │     }                                                                       │
   * │     // Expecting order to be saved...                                       │
   * │ }                                                                           │
   * │                                                                             │
   * │ KẾT QUẢ: 💥 UnexpectedRollbackException!                                    │
   * │ LÝ DO: process() đã mark TX là rollback-only                                │
   * │        Dù catch, khi commit vẫn rollback!                                   │
   * │                                                                             │
   * │ FIX: Dùng REQUIRES_NEW cho payment                                          │
   * │ @Transactional(propagation = Propagation.REQUIRES_NEW)                      │
   * │ void process() { }  // TX riêng, rollback không ảnh hưởng outer             │
   * │                                                                             │
   * │ SCENARIO 3: Audit log luôn save (REQUIRES_NEW)                              │
   * │ ─────────────────────────────────────────────────────────                   │
   * │ @Transactional                                                              │
   * │ void transfer(Account from, Account to, BigDecimal amount) {                │
   * │     auditService.log("Transfer started");  // Phải save dù fail             │
   ...
   * │ }  // Commit ngay, dù transfer() rollback                                   │
   * │                                                                             │
   * │ SCENARIO 4: Partial rollback (NESTED)                                       │
   * │ ─────────────────────────────────────────────────────────────               │
   * │ @Transactional                                                              │
   * │ void processItems(List<Item> items) {                                       │
   * │     for (Item item : items) {                                               │
   * │         try {                                                               │
   * │             itemService.process(item);  // NESTED                           │
   * │         } catch (Exception e) {                                             │
   * │             log.error("Item {} failed, skip", item.getId());                │
   * │             // Chỉ item này rollback, các item khác OK                      │
   * │         }                                                                   │
   * │     }                                                                       │
   * │ }                                                                           │
   * │                                                                             │
   ...
   * │     // Nếu fail → rollback to savepoint, outer TX tiếp tục                  │
   * │ }                                                                           │
   * └─────────────────────────────────────────────────────────────────────────────┘
   *
   * ┌─────────────────────────────────────────────────────────────────────────────┐
   ...
   * │                                                                             │
   * │ ┌──────────────┬────────────────┬─────────────────┬────────────────────────┐│
   * │ │ Propagation  │ Inner fail     │ Outer fail      │ Use case               ││
   * │ ├──────────────┼────────────────┼─────────────────┼────────────────────────┤│
   * │ │ REQUIRED     │ Both rollback  │ Both rollback   │ Default, same TX       ││
   * │ │ REQUIRES_NEW │ Only inner     │ Only outer      │ Independent ops        ││
   * │ │ NESTED       │ Inner only*    │ Both rollback   │ Partial rollback       ││
   * │ └──────────────┴────────────────┴─────────────────┴────────────────────────┘│
   * │                                                                             │
   * │ *NESTED: Inner rollback to savepoint, outer có thể continue                 │
   * │                                                                             │
   * │ CHỌN NÀO?                                                                   │
   * │ - REQUIRED: 99% cases, cùng unit of work                                    │
   * │ - REQUIRES_NEW: Audit, logging, independent records                         │
   * │ - NESTED: Batch processing với partial failure tolerance                    │
   * └─────────────────────────────────────────────────────────────────────────────┘
   *
   * ┌─────────────────────────────────────────────────────────────────────────────┐
   * │ GOTCHA: Self-invocation với Propagation                                     │
   * ├─────────────────────────────────────────────────────────────────────────────┤
   * │                                                                             │
   * │ ❌ SAI:                                                                     │
   * │ @Service                                                                    │
   ...
   * │     void createOrder() {                                                    │
   * │         save(order);                                                        │
   * │         this.audit("created");  // Self-invoke → Propagation IGNORED!       │
   * │     }                                                                       │
   * │                                                                             │
   * │     @Transactional(propagation = Propagation.REQUIRES_NEW)                  │
   * │     void audit(String action) {                                             │
   * │         // REQUIRES_NEW bị bỏ qua! Vẫn chạy trong TX cũ                     │
   * │     }                                                                       │
   * │ }                                                                           │
   * │                                                                             │
   * │ ✓ FIX: Tách ra service khác                                                 │
   * │ @Autowired AuditService auditService;                                       │
   * │ auditService.audit("created");  // Gọi qua proxy → REQUIRES_NEW work        │
   * └─────────────────────────────────────────────────────────────────────────────┘
   */
}
