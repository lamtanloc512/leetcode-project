...
public class BasicBitwiseExercises {
    ...
    public static int findComplement(int n) {
        // Cách tạo mask toàn 1 có độ dài bằng n:
        // Integer.highestOneBit(n) la bit 1 cao nhat.
        // Dịch trái 1 cái rồi trừ 1.
        if (n == 0) return 1;
        long mask = Integer.highestOneBit(n); 
        mask = (mask << 1) - 1; // 100 -> 1000 - 1 = 111
        return n ^ (int)mask;
    }
    
    // Note: Cách trên đơn giản hóa. Có thể dùng Integer.numberOfLeadingZeros

    /**
     * Bài 17: Binary Gap. Khoảng cách lớn nhất giữa hai số 1 bao quanh.
